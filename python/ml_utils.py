...
def preprocessar_dados(df_bruto):
    ...
    df = df_bruto.copy()
    # Força os nomes das colunas para minúsculo para bater com os modelos
...
def gerar_analise_exploratoria(df):
    ...
    sns.histplot(data=df_clean, x='umidade', kde=True, hue='chuva', multiple='stack', palette='Blues', ax=ax1)
    ...
    sns.heatmap(df_clean.corr(), annot=True, cmap='Coolwarm', fmt='.2f', linewidths=0.5, ax=ax3)
    ...
    sns.scatterplot(data=df_clean, x='umidade', y='ph', hue='chuva', palette='deep', ax=ax5)
...
def treinar_e_avaliar_modelos(df):
    ...
    # Gerando o gráfico de barras comparativo exigido na avaliação
    fig_comp, ax = plt.subplots(figsize=(8, 4))
    df_res.set_index('Modelo')[['Acurácia', 'F1-Score']].plot(kind='bar', ax=ax, color=['#3498db', '#e74c3c'])
